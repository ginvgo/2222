...
createApp({
    setup() {
        ...
        const newProjectForm = reactive({ folderName: '', name: '' });
        ...
        const createNewProject = async () => {
            if (!newProjectForm.folderName || !newProjectForm.name) {
                alert('请填写完整信息');
                return;
            }
            // Simple validation
            if (!/^[a-z0-9-]+$/i.test(newProjectForm.folderName)) {
                alert('文件夹名称格式不正确');
                return;
            }
            ...
            loading.value = true;
            try {
                // To create a new project, we send metadata with an initial content.md
                const formData = new FormData();
                const projectData = {
                    name: newProjectForm.name,
                    folderName: newProjectForm.folderName,
                    is_public: true,
                    is_encrypted: false,
                    passwords: [],
                    remember_duration: 30,
        ...
        return {
            ...
            showAddProjectModal, newProjectForm, createNewProject,
            ...
            showJsLibrary, jsLibrary, currentJsLib, createNewJs, selectJsLib, saveJsLib, deleteJsLib, toggleJsInjection, handleJsUpload,
