...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (username === validUsername && inputHash === validPasswordHash) {
      ...
      // Use SameSite=Lax for better compatibility with POST requests in some browsers
      response.headers.append('Set-Cookie', `admin_session=${token}; Path=/; HttpOnly; SameSite=Lax; Max-Age=86400`);
      return response;
    }
